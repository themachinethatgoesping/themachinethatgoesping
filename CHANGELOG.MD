...
Versioning scheme is currently based on my mood, but will be changed to [Semantic Versioning](https://semver.org/) once the project is more stable. 

# [Unreleased]

-echosounders -> 0.48.3
-pingprocessing -> 0.15.1
-widgets -> 0.1.5
-tools -> 0.33.1
-navigation -> 0.19.1

## Improvements
-echosounders: initial implementation for s7k
-pingprocessing/widgets: fixed reading geotif
-ai refractor tile builder
-navigation: convinient epgs functions in navtools
-echosounders: experimental support for kmall configurations: dual head, dual rx, dual tx dual rx
-echosoudners: kmall: accept unsorted navigations with a warning

## Fixes
- tools: fix printing nan/inf statistics
- echosounders: datagram container: fixed count_datagrams_per_type and find_datagram_types functions to return strings
- KMALL: deal with missing SYSTEM parameter in installation by allowing em710 etc to default to SingleTxSingleRx
- KMALL: more meaningfull error message for unknown datagram types

## Maintanance
- update frozen 1.1.1 -> git-master
- update fast_float 8.2.10 -> 8.3.0
- update nanobind 3.0.1 -> 3.1.0
- activate c++26 standard
- geographiclib wrap: stop injecting c++20 compile arguments into the dependency
- invalidated old kmall filepackage cache
- invalidated old navigation filepackage cache
...
## Improvements
- echosounders: kongsbergall xyz datagram: improved printing beam content
- echosounders: kongsbergall rawrangeandangle datagram: improved printing beam content
- echosounders: kongsbergall runtimeparameters datagram: added get_frequency_mode_in_hertz to guess frequency mode for em systems
- echosounders: kongsbergall runtimeparameters datagram: improved printing of frequency mode in hertz
...
- echosounders -> 0.48.1
- tools -> 0.33.0
- pingprocessing -> 0.15.0

## Improvements (Breaking)

- algorithms: adding beam trace class and simple 2D beam tracer
- algorithms/tools/navigation: rework ypr around quaternions (breaks API)
- navigation: rename positionaloffsets to sensorpose to reuse structure
- SensorConfiguration: compute_target_pose function to compute target pose from sensor data and reference heading
- Navigation: support for subposes for transducers etc.
- algorithm: towards raytracing
- tools: add register_table to objectprinter
- Navigation: reworking sensor configuration
- Echosounders: read more navigation configurations from .all and .kmall files
- Echosounders: convenience container for NetworkVelocityDatagram Attitudes
- echosiybders: convenience container for MWC tx sectors

## Fixes
- echosounders/navigation: fix motion reading confusion for .all
- echosounders/navigation: only apply attitude offsets if not already applied
- echosounders: fix race condition in file initialization
- pingprocessing: fix sorting issues with echogrambuilder combine backends

## Maintanance
- tools: update catch2 3.15.2 -> 3.16.0
- tools: update nanobind 2.13.0 -> 2.14.0
- tools: update robin-map 1.4.0 -> 1.4.1
- tools: update nanobind 2.14.0 -> 3.0.1
- tools: remove dependency on hinnant-date (c++20 features now available)
- removed pybind11 dependency (completed switch to nanobind)
- fixed minor warnings suhc as comparing signed and unsigned integers, and unused variables

## Breaking
- tools: removed ability to parse hinnant_data %numberS format, and switched to standard c++20 chrono formatting. 
...
## Fixes
- navigation: Geographiclib exceptions now caught before they cause crashes within openmp loops
- echosoudners: KMALL/MRZ: incidence angle computation used x and not y 

## Maintenance
- tools: update catch2 3.15.1 -> 3.15.2
...
## Fixes
- widgets: qt: fix crash caused by non ascii symbol characters used as labels
- widgets: jupyter widgets where not scaling correctly
...
- algorithms -> 0.11.2
- navigation -> 0.18.3
- pingprocessing -> 0.14.0
- widgets -> 0.1.3

## Fixes [ breaking change in Results]
- echosounders: fixed missing 10x in av pulse length correction
- echosounders: gridded mmap did not correctly store non-gridded axis information
- widgets: WCIViewer: fix time synchronization loop between different slots
- algorithms: resamplingfunction: deal with degenerated min/max sample ranges
- pingprocessing: combined and concatinated backends could not deal with axis changes (e.g swith from depht to range)

## Improvements
- echosounders: vectorized sub struct access for kmall SKBinary and .all attitude datagrams
- echosounders: fixed reading unsorted attitude datagrams (.kmall and .all)
- echosounders: add missing functions to kmallwatercolumn calibration (python interface)
- echosounders: kongsbergall datagram: implement tensor style access for beams substruture
- echosounders: kongsberg xyz: compute beam incidence angle horizontal plane (BAC) from x/y/z coordinates and beam incidence angle adjustment
- algorithms: esp3 port of bottom detection algorithm
- pingprocessing: additional meta information for pings (main frequency, pulse duration, sample interval)
- pingprocessing: revamping cross-calibraiton and layer processing
- echogrambuilder: max_gap option and new ping_distance axis
- echogrambuilder: add simple bottom_detection function to echogrambuilder
- widgets: support new distance axis for echogramviewer
- widgets: echogramviewer: remember last used settings for colorbar selector
- widgets: echogramviewer: AutoY now only scales to main layer (not complete picture)
- widgets: echogramviewerQt: performance improvement for interactive parameter editing
- widgets: echogramviewer: interact with wci stacks in echogramviewer


## Maintenance
- tools:
  - update fast_float 8.2.5 -> 8.2.10
  - update fmt 12.1.0 -> 12.2.0
  - update pugixml 1.15 -> 1.16
  - update nanobind 2.12.0 -> 2.13.0
...
## Improvements
- echosounders: added unittest for kmall file handler

## Fixes
- echosounders: fixed wrong handling of empty padding in kmall svp datagram
- widgets: fixed wrong import for TILE_SOURCES in mapviewer widgets

# [0.33.0] - 22-05-2026
- pingprocessing -> 0.13.0
- algorithms -> 0.11.1
- tools -> 0.31.2
- echosounders -> 0.47.0
- navigation -> 0.18.2
- widgets (new library) -> 0.1.1

## Highlights
- Handling kongsberg .kmall format is now supported in echosounders
- Vast speedup of echogram generation and plotting
- Ability to store echograms as memory-mapped files for faster loading and lower memory usage

## Improvements
- pingprocessing/overview/pingoverview: added speed calculation for pingoverview
- pingprocessing fixed wrong exclusion for layerprocessor remove_station functions
- complete overhaul of echogram and wci viewer widgets
- algorithms: added bubble backscatter models (Thuraisingham 1997, Li 2023)
- pingprocessing: overview compute/plot ping_rate_in_Hz
- echosounders: get_sensor_timestamp functions for KMBinary datagram
- pingprocessing: option to filter files by overlapping time ranges and areas (overlap_filter)
- various new filters for pingprocessing filter and split pings
- pingprocessing: add filter by heading
- pingprocessing echogrambuidler: allow oversampling for better plotting (mean of oversampled image)
- echosounders: towards calibration computation documentation
- pingprocessing: fixes for layers and new echogrambuilders
- pingprocessing: additional options for reprojecting navigation plots
- pingprocessing: Echogrambuilder add option to add custom axis
- pingprocessing: gridded echogram backend
- echosounders: added container overloads for selected kongsberg datagram substructures
- echosounders: get_sound_velocity_profile function for KongsbergAllFileData
- algorithms: added sound velocity profile interpolation functions
- pingprocessing - make_image: enable usage even if no navigation is available

## Performance
 - echosounders: improved initializaiton of navigation data interface when adding many files (fixed sorting issues)
- tools: made InputFileManager thread-safe via thread_local stream pools (zero synchronization overhead)
- echosounders: added mp_cores parameter to init_from_file for parallel per-file ping initialization (default: 1)
- echosounders: fix perfmance bug when indexing 1000+ file

## Fixes
- echosounders: check for active attitude sensor when parsing navigation data from kmall files (fixes wrong navigation for some files)
- fixed echogram layers for new echogrambuilder class
- pingprocessing: fixed echogram get_bottom_z not reacting to beam selection
- navigation: fixed wrong combined transmitter offset 

## Maintanance
- updated catch2 3.11.0 -> 3.15.1
- updated fast_float 8.1.0 -> 8.2.5
- updated nanobind 2.10.2 -> 2.12.0
- moving to experimental xtensor-python nanobind branch
- updated xtl 0.8.1 -> 0.8.2
...
## Improvements
- echosounders/kmall: support for PortableMKIIHead transducer configuration
- echosounders/kmall: support for version 0 and 1 of MRZPingInfo structure
- echosounders/kmall: support for 12 bytes long MWCRXBeamData structure
- echosounders/kmall: support for phase data in MWCRXBeamData structure

## [0.32.0] - 15-12-2025

- echosounders -> 0.46.1
- tools -> 0.31.1
- pingprocessing -> 0.12.1
- navigation -> 0.18.1
- algorithms -> 0.11.0

## Improvements

- echosounders: initial support for kmall reading
- echosoudners/kongsbergall: use frozen unordered_map to speed up installtionparamter lookups
- echosounders: new generative get_wci and get_wci_correction functions
- pingprocessing: export functions for pyqtgraph_viewer
- tools: printer for bivectorinterpolators
- tools: new downsampling function for 1d vectors
- echsounders: simpler get_navigation convenience functions
- navigation: simpler get_navigation convenience functions
- new geolocationvector structures for faster python access
- support for python 3.14
- algorithms: option class for TxSignalType
- echosounders: kongsbergall: support absorption per tx sector
- echosounders/algorithms: watercolumn calibration: support absorption per beam

## Fixes

- echosounders: Reintroduced Simradraw string to datagram identifier conversion
- tools: added missing xtensor instantiations

## Maintanance
- tools: many objectprinter functions now take string_views
- update fmt 12.0.0 -> 12.1.0
- update eigen 3.4.0 -> 5.0.1
- update xsimd 13.2.0 -> 14.0.0
- cleanup: remove old pymodule files from echosounders
- cleanup: remove annotation data interface
- updated nanobind 2.9.2 -> 2.10.2
- towards python 3.14 support
- use github version of pybind11_mkdoc (switches comment logic fom __doc_ to mkd_doc)

## [0.31.0] - 23-10-2025

- tools -> 0.31.0
- algorithms -> 0.10.0
- pingprocessing -> 0.12.0
- echosounders -> 0.46.0
- pingprocessing -> 0.12.0
- navigation -> 0.18.0
- gridding -> 0.5.0
...
- algorithms: added simple 2d backward mappers based on xtensor
- tools:bivecorinterpolator: added insert_row function
- tools add nan extrapolation mode for float vector interpolators
- added Option class as convenience wrapper for enums
- created nanobind version of tools (pybind11 version is still default)
- tools: more overloads for bivectorinterpolator with xtensor input
- echosounders: amplitude calibration functions now support beam and sample varying offsets (as matrix)
- pingprocessing: mp_cores parameter for get_av / get_sv functions
- kongsbergall: add pre_hash function to kongsberg calibration and update_calibration to watercolumn_calibration
...
- updated xtl 0.8.0 -> 0.8.1
#- update geographiclib 2.5.2 -> 2.6.0 (postponed because of build issues)
- echosounders:
  - refractored KongsbergAllWaterColumnCalibration into cpp file
- mooving to pre-compiled headers
...
### Maintanance
  ...
  - set warning_level=0 to silence external warnings
